...
function RequestAsset() {
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.filter(item => {
      ...
      const matchesType =
        typeFilter === "all" ? true : item.productType === typeFilter
    ...
    <div className="bg-[var(--bg-page)] -m-4 p-4 md:p-6 min-h-screen">
      <div className="max-w-6xl space-y-5">
        ...
        {filteredAssets.length === 0 ? (
          <div className="rounded-xl border border-[var(--border)] bg-white px-4 py-10 text-center text-sm text-[var(--text-secondary)]">
            No assets available.
          </div>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filteredAssets.map(item => {
              ...
              return (
                <div
                  key={item._id}
                  className="rounded-xl border border-[var(--border)] bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow"
                >
                  <div className="p-4">
